...
public class M5P extends M5Base implements Drawable
{
	...
	{
		RuleNode r = getM5RootNode();
		PreConstructedLinearModel m = r.getModel();
		//System.out.println("leaf " + m.);
		System.out.println("root model " + m);
		System.out.println("rule set size " + getRuleSet().size());

		if (getRuleSet().size() == 1)
		{
			double c[] = m.coefficients();
