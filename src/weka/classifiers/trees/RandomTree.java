 ...
 */
public class RandomTree extends AbstractClassifier implements OptionHandler, WeightedInstancesHandler, Randomizable,
		Drawable
{
	...
	public String globalInfo()
	{
		...
		return "Class for constructing a tree that considers K randomly "
				...
				+ " an option to allow estimation of class probabilities based on" + " a hold-out set (backfitting).";
	}
	...
	public void buildClassifier(Instances data) throws Exception
	{
		...
		// Build tree 
		buildTree(train, classProbs, new Instances(data, 0), m_MinNum, m_Debug, attIndicesWindow, rand, 0,
	 ...
	 *             if generation fails
	 */
	protected void buildTree(Instances data, double[] classProbs, Instances header, double minNum, boolean debug,
			int[] attIndicesWindow, Random random, int depth, boolean allow) throws Exception
	{
		...
		if (Utils.gr(vals[m_Attribute], 0))
		{
			...
			for (int i = 0; i < distribution.length; i++)
			{
				...
				m_Successors[i].buildTree(subsets[i], distribution[i], header, m_MinNum, m_Debug, attIndicesWindow,
						random, depth + 1, allow);
			}
	...
	protected int toGraph(StringBuffer text, int num, RandomTree parent) throws Exception
	{
		...
		if (m_Attribute == -1)
		{
			text.append("N" + Integer.toHexString(RandomTree.this.hashCode()) + " [label=\"" + num + leafString()
					+ "\"" + " shape=box]\n");

		}
		else
		{
			text.append("N" + Integer.toHexString(RandomTree.this.hashCode()) + " [label=\"" + num + ": "
					+ m_Info.attribute(m_Attribute).name() + "\"]\n");
			for (int i = 0; i < m_Successors.length; i++)
			{
				text.append("N" + Integer.toHexString(RandomTree.this.hashCode()) + "->" + "N"
						+ Integer.toHexString(m_Successors[i].hashCode()) + " [label=\"");
				if (m_Info.attribute(m_Attribute).isNumeric())
				{
					if (i == 0)
