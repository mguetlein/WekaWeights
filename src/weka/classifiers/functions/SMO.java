...
public class SMO extends AbstractClassifier implements WeightedInstancesHandler, TechnicalInformationHandler
{
	public double[] getAttributeWeights()
	{
		...
		if (num != 1)
		{
			System.err.println("not that easy! not-null: " + num + " dim: " + m_classifiers.length + "x"
					+ m_classifiers[0].length);
			return new double[0];
		}

		//		System.out.println(i1);
		//		System.out.println(i2);
		//		System.out.println(m_classifiers[i1][i2].m_sparseWeights);
		double d[] = new double[m_classifiers[i1][i2].m_sparseWeights.length];
		for (int i = 0; i < d.length; i++)
			d[i] = Math.abs(m_classifiers[i1][i2].m_sparseWeights[i]);
	...
	public String[] getAttributes()
	{
		...
		String[] a = new String[m_classifiers[i1][i2].m_sparseIndices.length];
		for (int i = 0; i < a.length; i++)
			a[i] = m_classifiers[i1][i2].m_data.attribute(m_classifiers[i1][i2].m_sparseIndices[i]).name();
		return a;
	}
	...
	public class BinarySMO implements Serializable
	{
		...
		protected boolean takeStep(int i1, int i2, double F2) throws Exception
		{
			...
			else
			{
				...
				Lobj = (gamma - s * L) + L - 0.5 * k11 * (gamma - s * L) * (gamma - s * L) - 0.5 * k22 * L * L - s
						* k12 * (gamma - s * L) * L - y1 * (gamma - s * L) * v1 - y2 * L * v2;
				Hobj = (gamma - s * H) + H - 0.5 * k11 * (gamma - s * H) * (gamma - s * H) - 0.5 * k22 * H * H - s
			...
			for (int j = m_I0.getNext(-1); j != -1; j = m_I0.getNext(j))
			{
				if ((j != i1) && (j != i2))
				{
					m_errors[j] += y1 * (a1 - alph1) * m_kernel.eval(i1, j, m_data.instance(i1)) + y2 * (a2 - alph2)
